...
fn rsaprivdec_handler(ns :NameSpaceEx,_optargset :Option<Arc<RefCell<dyn ArgSetImpl>>>,_ctx :Option<Arc<RefCell<dyn Any>>>) -> Result<(),Box<dyn Error>> {  
    ...
    for f in sarr.iter() {
        ...
        let privkey = get_rsa_private_key(&code,passin.as_bytes())?;
        let mut f = std::io::stderr();
...
fn rsaverify_handler(ns :NameSpaceEx,_optargset :Option<Arc<RefCell<dyn ArgSetImpl>>>,_ctx :Option<Arc<RefCell<dyn Any>>>) -> Result<(),Box<dyn Error>> {   
    ...
    let privkey = get_rsa_private_key_base(&keyfile,passin.as_bytes())?;
    ...
    for f in sarr.iter() {
        ...
        for bagv in octdata.val.iter() {
            ...
            if types == OID_PKCS8_SHROUDED_KEY_BAG {
                ...
                let v8 = x509sig.encode_asn1()?;
                let ores = get_rsa_private_key(&v8,passin.as_bytes());
                if ores.is_ok() {
                    let pkey = ores.unwrap();
                    let _ = pkey.print_asn1("pkey", 0, &mut f)?;    
                } else {
                    let pkey = get_ec_private_key(&v8,passin.as_bytes())?;
                    let _ = pkey.print_asn1("pkey ec", 0, &mut f)?;
                }
...
fn pkcs7signerdec_handler(ns :NameSpaceEx,_optargset :Option<Arc<RefCell<dyn ArgSetImpl>>>,_ctx :Option<Arc<RefCell<dyn Any>>>) -> Result<(),Box<dyn Error>> {  
    let sarr :Vec<String>;
    let mut serr = std::io::stderr();
    init_log(ns.clone())?;
    sarr = ns.get_array("subnargs");
    for f in sarr.iter() {
        let v8 = read_file_bytes(f)?;
        let mut signer :Asn1Pkcs7SignerInfo = Asn1Pkcs7SignerInfo::init_asn1();
        let _ = signer.decode_asn1(&v8)?;
        signer.print_asn1("",0,&mut serr)?;
    }
    Ok(())
}
...
pub fn load_pkcs7_handler(parser :ExtArgsParser) -> Result<(),Box<dyn Error>> {
    let cmdline = r#"
    {
        ...
        "pkcs7signerdec<pkcs7signerdec_handler>##signerfile ... to decode signer file##" : {
            "$" : "+"
        }
    }
    "#;
