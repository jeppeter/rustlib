...
pub fn des_pure_encrypt(data: &[u8],key: &[u8], _iv: &[u8])->Result<Vec<u8>,Box<dyn Error>>{
    ...
    if (data.len() % alignsize) != 0 {
        clen = ((data.len() + alignsize - 1 ) / alignsize) * alignsize;
    } else {
...
pub fn des_ede3_pure_encrypt(data: &[u8],key: &[u8], _iv: &[u8])->Result<Vec<u8>,Box<dyn Error>>{
    let mut retdata  :Vec<u8> = Vec::new();
    let clen :usize;
    let alignsize :usize = des::TdesEde3::key_size();
    for i in 0..data.len() {
        retdata.push(data[i]);
    }
    if (data.len() % alignsize) != 0 {
    ...
    debug_trace!("data.len {} clen {}",data.len(),clen);
    while retdata.len() < clen {
        retdata.push(0);
    }
    if key.len() < alignsize {
    ...
    let ck = des::TdesEde3::new(bkey.into());
    debug_trace!("ck {:?} alignsize {}",ck,alignsize);
    debug_trace!("retdata.len {} data.len {}",retdata.len(), data.len());

    let eo = ck.encrypt_padded_mut::<cipher::block_padding::Pkcs7>(&mut retdata,data.len());
    if eo.is_err() {
        let e = eo.err().unwrap();
        extargs_new_error!{AesLibError,"encrypt error {}", e}
    }
...
pub fn des_ede3_pure_decrypt(encrypted_data: &[u8], key: &[u8], _iv: &[u8]) -> Result<Vec<u8>,Box<dyn Error>> {
    let mut retdata :Vec<u8> = Vec::new();
    let alignsize :usize = des::TdesEde3::key_size();
    for i in 0..encrypted_data.len() {
        retdata.push(encrypted_data[i]);
...
// Encrypt a buffer with the given key and iv using AES-256/CBC/Pkcs encryption.
pub fn aes128_cbc_encrypt(data: &[u8],key: &[u8], iv: &[u8])->Result<Vec<u8>,Box<dyn Error>>{
    let mut encryptor=crypto::aes::cbc_encryptor(
...
// Decrypts a buffer with the given key and iv using AES-256/CBC/Pkcs encryption.
pub fn aes128_cbc_decrypt(encrypted_data: &[u8], key: &[u8], iv: &[u8]) -> Result<Vec<u8>,Box<dyn Error>> {
    let mut decryptor = crypto::aes::cbc_decryptor(
        crypto::aes::KeySize::KeySize128,
        key,
        iv,
